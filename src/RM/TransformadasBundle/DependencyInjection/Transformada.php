...
class Transformada
{
    ...
    {
        $this->em = $GLOBALS['kernel']->getContainer()->get('doctrine')->getManager($_SESSION['connection']);
    }

    public function getTransformadas($nombre = '', $tipoVar = 0)
    {
        $registros = $this->em->getRepository('RMTransformadasBundle:Vt')->obtenerVariablesTransformadas($nombre,
            $tipoVar);
        return $registros;
    }
    ...
    public function getSegGrupoVIbyId($id_vt)
    {
        $repo = $this->em->getRepository('RMTransformadasBundle:Vt');
        $registros = $repo->obtenerSegGrupoVIbyId($id_vt);
    ...
    public function eliminarSegGrupoVIbyPost(
        $listaSegmentosAEliminar,
        $listaGruposAEliminar,
        $listaIntervalosAEliminar,
        $id_vt
    ) {
